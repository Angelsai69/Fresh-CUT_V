...
import { ChevronDown, Sun, Moon, MapPin, Plus, X, Star, Eye, EyeOff } from 'lucide-react'
...
// ── Field style helper ────────────────────────────────────────
const fieldStyle = (t) => ({
  flex: 1, width: '100%', padding: '9px 12px',
  borderRadius: tokens.radius.sm,
  border: `1px solid ${t.border}`,
  background: t.isDark ? 'rgba(255,255,255,0.06)' : tokens.colors.bgLight,
  color: t.text, fontSize: '13px', fontWeight: 450,
...
function InfoField({ label, icon, children, last }) {
  return (
    <div style={{
      ...
      borderBottom: last ? 'none' : '1px solid rgba(15,43,70,0.06)',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
        <span style={{ fontSize: 13 }}>{icon}</span>
        <span style={{ fontSize: '11px', fontWeight: 600, color: 'inherit', opacity: 0.55, letterSpacing: '0.5px', textTransform: 'uppercase' }}>
          {label}
        </span>
      </div>
      {children}
...
// ── Accordion wrapper ─────────────────────────────────────────
// header: the always-visible summary row
// children: the collapsible body
...
function AccordionHeader({ t, label, meta, open }) {
  return (
    <div style={{
      ...
      background: t.card,
      borderRadius: open ? `${tokens.radius.card} ${tokens.radius.card} 0 0` : tokens.radius.card,
      padding: '14px 18px',
      boxShadow: t.shadow1,
      ...
      <motion.div
        ...
        transition={tokens.motion.fast}
        style={{ color: t.textSub, display: 'flex' }}
      >
...
export function ProfileScreen({ t, dark, setDark }) {
  ...
  const addLocation    = () => {
    ...
    setLocations(l => [...l, {
      id: Date.now(), name: newLabel,
      address: newAddr || 'Address not set', emoji: '📍', primary: false,
    }])
  ...
  return (
    ...
    >
      ...
      <Accordion
        ...
        header={(open) => (
          <AccordionHeader
            t={t}
            label="Account Info"
            meta={`${userInfo.username} · ${userInfo.diet !== 'None' ? userInfo.diet : 'No diet set'}`}
            open={open}
          />
        )}
      >
        <div style={{
          ...
          <InfoField label="Cell Number" icon="📱">
            <input value={userInfo.phone} onChange={e => setField('phone', e.target.value)} style={fieldStyle(t)} />
          </InfoField>
          ...
          <InfoField label="Password" icon="🔒">
            <div style={{ position: 'relative', flex: 1 }}>
              <input
                type={userInfo.showPass ? 'text' : 'password'}
                defaultValue="supersecret123"
                style={{ ...fieldStyle(t), paddingRight: 36 }}
              />
              <button onClick={() => setField('showPass', !userInfo.showPass)} style={{
          ...
          <InfoField label="Special Diet" icon="🥗" last>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, paddingTop: 4 }}>
              {DIET_OPTIONS.map(opt => {
                const active = userInfo.diet === opt
                return (
                  <motion.button key={opt} whileTap={{ scale: 0.92 }} onClick={() => setField('diet', opt)} style={{
                    padding: '5px 13px', borderRadius: tokens.radius.full,
                    border: `1.5px solid ${active ? tokens.colors.mintStart : t.border}`,
                    background: active ? `linear-gradient(135deg, ${tokens.colors.mintStart}22, ${tokens.colors.mintEnd}18)` : 'transparent',
                    cursor: 'pointer', fontSize: '12px', fontWeight: active ? 700 : 450,
          ...
          <motion.button whileTap={{ scale: 0.97 }} style={{
            ...
            borderRadius: tokens.radius.card, background: mintGradient,
      ...
      {/* ── ACCORDION: Saved Locations ── */}
      <Accordion
        open={locOpen}
        onToggle={() => { setLocOpen(o => !o); setAddingNew(false) }}
        header={(open) => (
          <AccordionHeader
            t={t}
            label="Saved Locations"
            meta={primaryLoc ? `${primaryLoc.emoji} ${primaryLoc.name} is primary · ${locations.length} saved` : `${locations.length} saved`}
            open={open}
          />
        )}
      >
            ...
            <AnimatePresence>
              {locations.map((loc) => (
                <motion.div
                  ...
                  transition={tokens.motion.standard}
                  style={{
                    background: loc.primary ? tokens.colors.navy : (t.isDark ? 'rgba(255,255,255,0.05)' : tokens.colors.bgLight),
                ...
                >
                  <div style={{
                    ...
                    background: loc.primary ? 'rgba(123,232,200,0.2)' : `${tokens.colors.mintStart}15`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <p style={{ fontSize: '14px', fontWeight: 600, margin: 0, color: loc.primary ? 'white' : t.text }}>
                        {loc.name}
                      </p>
                      {loc.primary && (
                        <span style={{
                          fontSize: '9px', fontWeight: 700, background: mintGradient,
                          color: tokens.colors.navy, padding: '2px 7px',
                          borderRadius: tokens.radius.full, letterSpacing: '0.4px',
                        }}>PRIMARY</span>
                      )}
                    </div>
                    <p style={{
                      fontSize: '11px', fontWeight: 450, margin: '2px 0 0',
                      color: loc.primary ? 'rgba(255,255,255,0.58)' : t.textSub,
                  ...
                  {!loc.primary && (
                    <div style={{ display: 'flex', gap: 5, flexShrink: 0 }}>
                      <motion.button whileTap={{ scale: 0.84 }} onClick={() => setPrimary(loc.id)} title="Set primary"
                        style={{
                          width: 26, height: 26, borderRadius: tokens.radius.full,
                      ...
                      <motion.button whileTap={{ scale: 0.84 }} onClick={() => removeLocation(loc.id)}
                        style={{
                          width: 26, height: 26, borderRadius: tokens.radius.full,
                          background: `${tokens.colors.coral}14`, border: `1px solid ${tokens.colors.coral}28`,
                          cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          {!addingNew && (
            <motion.button
              ...
              style={{
                ...
                borderRadius: tokens.radius.card,
                border: `1.5px dashed ${tokens.colors.mintStart}50`,
                background: 'transparent', cursor: 'pointer',
                fontSize: '13px', fontWeight: 600,
                color: tokens.colors.mintEnd, fontFamily: 'inherit',
            ...
            {addingNew && (
              ...
              >
                <div style={{
                  ...
                  borderRadius: tokens.radius.card, padding: '14px',
                  ...
                  {['Label (e.g. Gym)', 'Address or ZIP'].map((ph, i) => (
                    <input key={ph} placeholder={ph}
                      ...
                      style={{
                        width: '100%', padding: '9px 12px', borderRadius: tokens.radius.sm,
                        border: `1px solid ${t.border}`, background: t.card, color: t.text,
                        fontSize: '13px', fontWeight: 450, fontFamily: 'inherit',
                        outline: 'none', marginBottom: i === 0 ? 7 : 10, boxSizing: 'border-box',
                      }}
                    />
                  ))}
                  <div style={{ display: 'flex', gap: 7 }}>
                    <motion.button whileTap={{ scale: 0.95 }} onClick={addLocation} style={{
                      flex: 1, padding: '9px', borderRadius: tokens.radius.sm,
                    ...
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => setAddingNew(false)} style={{
                      padding: '9px 14px', borderRadius: tokens.radius.sm,
      ...
      <Accordion
        ...
        header={(open) => (
          <AccordionHeader
            t={t}
            label="Bonus Rewards"
            meta={`$15.00 balance · ${completedBonus}/${BONUSES.length} complete`}
            open={open}
          />
        )}
      >
        <div style={{
          ...
          <div style={{
            ...
            borderRadius: tokens.radius.card, padding: '16px 18px',
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            marginBottom: 12, marginTop: 4,
          }}>
            <div>
              <p style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.7)', margin: '0 0 3px', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                Credit Balance
              </p>
              <p style={{ fontSize: '26px', fontWeight: 700, color: 'white', margin: 0, letterSpacing: '-0.4px' }}>
                $15.00
              </p>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {BONUSES.map((b, idx) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: idx * 0.05, ...tokens.motion.standard }}
                style={{
                  background: t.isDark ? 'rgba(255,255,255,0.05)' : tokens.colors.bgLight,
                  borderRadius: tokens.radius.card, padding: '14px 16px',
                  border: `1px solid ${t.isDark ? 'rgba(255,255,255,0.07)' : 'rgba(15,43,70,0.06)'}`,
                }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  <div style={{
                    width: 36, height: 36, borderRadius: tokens.radius.sm, fontSize: 18,
                    background: `${tokens.colors.mintStart}18`,
                  ...
                  <span style={{
                    ...
                    padding: '3px 9px', borderRadius: tokens.radius.full,
                  }}>{b.reward}</span>
                </div>
                <div style={{ height: 4, borderRadius: tokens.radius.full, background: `${tokens.colors.mintStart}22`, overflow: 'hidden' }}>
                  <motion.div
                    ...
                    transition={{ delay: 0.15 + idx * 0.05, duration: 0.5, ease: [0.4, 0, 0.2, 1] }}
                    style={{
                ...
                <p style={{ fontSize: '10px', fontWeight: 450, color: t.textSub, margin: '4px 0 0', textAlign: 'right' }}>
                  {b.progress}%
                </p>
              </motion.div>
            ))}
