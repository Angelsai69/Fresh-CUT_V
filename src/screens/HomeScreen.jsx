...
// to avoid layout thrashing on iOS Safari. will-change: transform is scoped
// via CSS class (.is-scrolling .parallax-subtitle) so the GPU layer is only
// promoted during active scroll, then released — saves memory on low-RAM devices.
function ParallaxHero({ t, onBonusOpen }) {
  ...
  const onScroll = useCallback(() => {
    // Add class immediately on scroll start to promote layers
    document.body.classList.add('is-scrolling')

    // Debounce class removal — 150ms after scroll stops
  ...
  return (
    <section style={{
      ...
      <div aria-hidden="true" style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden', borderRadius: 'inherit' }}>
        ...
        <motion.div
          ...
          style={{
            ...
            borderRadius: '50%',
            background: t.isDark
              ? `radial-gradient(circle, ${tokens.colors.mintStart}22 0%, transparent 70%)`
              : `radial-gradient(circle, ${tokens.colors.mintStart}30 0%, transparent 70%)`,
            filter: 'blur(48px)',
        ...
        <motion.div
          ...
          style={{
            ...
            borderRadius: '50%',
            background: t.isDark
              ? `radial-gradient(circle, ${tokens.colors.lemon}18 0%, transparent 70%)`
              : `radial-gradient(circle, ${tokens.colors.lemon}25 0%, transparent 70%)`,
            filter: 'blur(56px)',
            willChange: 'transform',
          }}
        />
      ...
      {/* Subtitle — will-change scoped via CSS .is-scrolling .parallax-subtitle */}
      <motion.p
        ref={subtitleRef}
